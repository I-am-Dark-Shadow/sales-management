...
const MyProgressPage = () => {
    ...
    useEffect(() => {
        const fetchSales = async () => {
            setIsLoading(true);
            try {
                const { data } = await axiosInstance.get('/api/sales/my-sales', { params: filters });
                setSales(data);
            } catch (error) {
    ...
    const handleExport = async (format, year, month) => {
        const toastId = toast.loading(`Exporting ${format.toUpperCase()}...`);
        try {
            const response = await axiosInstance.get(`/api/reports/my-sales/${format}?year=${year}&month=${month}`, { responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `my-sales-${year}-${month}.${format}`);
    ...
    return (
        ...
        <div className="rounded-2xl border border-black/10 bg-white p-5">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-black/10 pb-4">
                <h2 className="text-xl font-semibold tracking-tight">My Sales Progress</h2>
                ...
                <table className="w-full text-sm">
                    <thead className="bg-gray-light">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-semibold text-left">Products</th>
                    ...
                    <tbody>
                        ...
                        )) : (<tr><td colSpan="4" className="text-center py-8 text-gray-medium">No sales found for the selected period.</td></tr>)}
                    </tbody>
                    <tfoot>
